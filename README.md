...
Статус: **M5 — вау-момент готов** (кнопка «Расчёт» вызывает `IWaterPredictor` прямо в UI,
сравнение с реальным прогоном, живой счётчик точности, запись каждого прогона в `runs`,
счётчик переживает перезапуск). Дашборд метрик и история — в M6 (см. ТЗ, раздел 9).
